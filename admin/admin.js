...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  async function initMarketing() {
    ...
    if (dotEl && txtEl) {
      // En Vercel no sabemos desde el frontend si la key existe hasta que enviemos.
      // Asumimos que está operativo si hemos subido a Vercel.
  ...
  if (btnPrueba) {
    btnPrueba.addEventListener('click', async () => {
      ...
      const firma   = document.getElementById('mkt-firma')?.value.trim() || 'IAparaseniors';
  ...
  if (btnMasivo) {
    btnMasivo.addEventListener('click', async () => {
      ...
      for (const s of simpatizantesCache) {
        try {
          await sendBrevoEmail(s.email, s.name, asunto, cuerpo, firma);
          enviados++;
        } catch(err) {
          errores++;
